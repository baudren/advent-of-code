...
def matches(test_value, remaining, current):
    if current > test_value:
        return False
    if not remaining:
        return current == test_value
    else:
        return matches(test_value, remaining[1:], current+remaining[0]) or matches(test_value, remaining[1:], current*remaining[0])

def sol1(data):
    total = 0
    for line in data:
        test_value, elems = line.split(": ")
        test_value = int(test_value)
        elems = [int(e) for e in elems.split(" ")]
        current = elems[0]
        if matches(test_value, elems[1:], current):
            total += test_value
    return total


def matches2(test_value, remaining, current):
    if current > test_value:
        return False
    if not remaining:
        return current == test_value
    else:
        return matches2(test_value, remaining[1:], current+remaining[0]) or matches2(test_value, remaining[1:], current*remaining[0]) or matches2(test_value, remaining[1:], int(str(current)+str(remaining[0])))
...
def sol2(data):
    ...
    for line in data:
        ...
        elems = [int(e) for e in elems.split(" ")]
        current = elems[0]
        if matches2(test_value, elems[1:], current):
            total += test_value
    return total
