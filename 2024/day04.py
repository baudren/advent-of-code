...
def sol1(data):
    ...
    for i, line in enumerate(grid):
        for j, letter in enumerate(line):
            if letter == 'X':
                for dir_ in dirs:
                    if find_xmas(grid, (i, j), dir_):
                        total += 1
    return total


def find_x_mas(grid, pos):
    x, y = pos
    if grid[x][y] != 'A':
        return 0
    if x == 0 or x == len(grid)-1 or y == 0 or y == len(grid[0])-1:
        return 0
    if set([grid[x-1][y-1], grid[x+1][y-1], grid[x-1][y+1], grid[x+1][y+1]]) != set(['M', 'S']):
        return 0
    if grid[x-1][y-1] == grid[x+1][y+1]: return 0
    if grid[x-1][y+1] == grid[x+1][y-1]: return 0
    return 1
...
def sol2(data):
    ...
    for line in data:
        grid.append([e for e in line.strip()])
    return sum([find_x_mas(grid, (i, j)) for j in range(len(data[0])) for i in range(len(data))])
