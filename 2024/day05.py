...
def sol1(data):
    ...
    total = 0
    for i in range(limit+1, len(data)):
        line = [int(e) for e in data[i].split(",")]
...
def is_valid(rules, line):
    indices = {
            k: i for i, k in enumerate(line)
        }
    for before in line:
        for after in rules.get(before, []):
            if after in indices:
                if indices[after] < indices[before]:
                    return False
    return True

def find_invalid(rules, line):
    indices = {
            k: i for i, k in enumerate(line)
        }
    for before in line:
...
def sol2(data):
    ...
    for i in range(limit+1, len(data)):
        ...
        if not is_valid(rules, line):
            while True:
                indices, before, after = find_invalid(rules, line)
                line[indices[before]], line[indices[after]] = line[indices[after]], line[indices[before]]
                if is_valid(rules, line):
                    total += line[len(line)//2]
                    break
    return total
